<template>
    <!-- 添加任务面板 -->
    <div class='task' ref="task" draggable='true' v-if="addTaskBlankIsShow" @click="changeTaskValue">
        <!-- 标签 -->
        <div class='task__tags'>
            <button @click="changeTask" style="cursor: pointer;"  class='task__options'>
    ...
    <div class="button" v-if="addTaskBlankIsShow" style="margin-left:30%">
        <button @click="cancelTaskValue" >取消</button>
        <el-button @click="addTaskValue" :disabled="form.listvalue === ''">添加</el-button>
    </div>

    <!-- 添加任务按钮 -->
    <div class='task' draggable='true'  v-if="addTaskButtonIsShow" >
        <!-- 标签 -->
        <div style="height:20px; text-align:center; line-height:20px; ">
            <!-- 添加任务 -->
            <button @click="addTask" class='project-participants__add'></button>
            <span @click="addTask">添加任务</span>
        </div>
    </div>
    
    <!-- 添加任务弹出框 -->
    <el-dialog v-model="props.taskVisible" title="Your任务 | 亲" @close="closeDialog">
        <div class="content">
            <el-input 
                v-model="form.listvalue" 
                type="textarea" 
        ...
        <!-- 标签 -->
        <div class="tagsTag">
            <i class="iconfont" v-if="tagsTags.length">&#xe61d;</i>
            <el-tag
                v-for="(tag,index) in tagsTags"
                :key="index"
                class="mx-1"
                closable
                round
...
import { dropdownMenuProps, ElMessage  } from 'element-plus'

const addTaskBlankIsShow = ref(false)
const addTaskButtonIsShow = ref(false)
// const taskVisible = ref(false);
// const taskVisible = defineProps(['taskVisible']);
const timePick = ref('');
// const area = defineProps(['area']);
let props = defineProps(['area'])
...
const handleInputConfirm = () => {
    if (inputValue.value.tagName) {
        tagsTags.value.push(inputValue.value)
    }
